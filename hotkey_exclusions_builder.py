...
def find_matching_names(json_data):
    ...
    for name_key in names_data.keys():
        ...
        if not description:
            continue
        m1 = (not len(names_to_match) or any(match in name_value for match in names_to_match))
        m2 = (not len(descriptions_to_match) or any(match in description for match in descriptions_to_match))
